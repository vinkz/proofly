  ...
  return (
    <main className="min-h-screen bg-[#f5f2ea] text-slate-950">
      <section className="mx-auto flex min-h-screen w-full max-w-5xl flex-col px-4 py-5 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-3 border-b border-slate-950/10 pb-4">
          <p className="text-sm font-semibold tracking-tight">CertNow</p>
          <Badge variant="brand" className="rounded-full">
            Gas Safe record
          </Badge>
        </div>

        <div className="grid flex-1 gap-8 py-8 lg:grid-cols-[1.25fr,0.75fr] lg:items-end">
          <div>
        ...
        <section className="grid gap-4 border-t border-slate-950/10 py-6 md:grid-cols-2">
          <div>
            ...
            <div className="mt-3 space-y-2">
              {job.certificates.length ? (
                ...
                <p className="rounded-2xl bg-white/65 px-4 py-3 text-sm text-slate-600">Certificates will appear here when available.</p>
              )}
            </div>
          </div>
          ...
          <div>
            {job.engineerOwnsJob ? (
              <div className="rounded-3xl bg-slate-950 p-5 text-white">
                <p className="text-sm font-semibold">Engineer progress</p>
            ...
            {job.landlordHasMultipleJobs ? (
              <div className="mt-4 rounded-3xl bg-white/70 p-5">
                <p className="text-sm font-semibold">Want all certificates in one place?</p>
                <p className="mt-1 text-sm text-slate-600">
                  This email is linked to more than one job. A free landlord account will show all properties and certificates together.
                </p>
