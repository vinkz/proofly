...
export default async function DocumentsPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const { data } = await supabase
    .from('reports')
    .select('id, job_id, storage_path, generated_at, created_at, kind, jobs:jobs(client_name, title, status, scheduled_for)')
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-2">
        ...
        <h1 className="text-3xl font-semibold text-muted">Documents</h1>
        <p className="text-sm text-muted-foreground/70">Search, preview, and share generated PDFs.</p>
      </header>

      <form className="rounded-3xl border border-white/20 bg-white/80 p-4" action="/documents" method="get">
        <input
          type="text"
          name="q"
          defaultValue={query}
