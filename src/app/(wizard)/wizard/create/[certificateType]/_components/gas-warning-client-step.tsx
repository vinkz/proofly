...
type GasWarningClientStepProps = {
  clients: ClientListItem[];
  totalSteps: number;
};

export function GasWarningClientStep({ clients, totalSteps }: GasWarningClientStepProps) {
  ...
  const redirectToWizard = (jobId: string) => {
    router.push(`/wizard/create/gas_warning_notice?jobId=${jobId}&clientStep=1`);
  };
  ...
  const handleSelect = (clientId: string) => {
    startTransition(async () => {
      try {
        ...
        const { jobId: createdJobId } = await createJob({ certificateType: 'gas_warning_notice', clientId });
  ...
  const onSubmit = handleSubmit((values) => {
    startTransition(async () => {
      try {
        ...
        const { jobId: createdJobId } = await createJob({ certificateType: 'gas_warning_notice', clientId: id });
        setJobId(createdJobId);
        setShowNewClientForm(false);
        applyClientDefaults(newClient);
        pushToast({ title: 'Client created', variant: 'success' });
  ...
  const canContinue =
    warningInfo.customer_name.trim().length > 0 &&
    warningInfo.property_address.trim().length > 0 &&
    warningInfo.postcode.trim().length > 0 &&
    warningInfo.customer_contact.trim().length > 0;
    ...
    startTransition(async () => {
      try {
        await saveGasWarningJobInfo({
          jobId,
          data: {
            property_address: warningInfo.property_address,
            postcode: warningInfo.postcode,
            customer_name: warningInfo.customer_name,
            customer_contact: warningInfo.customer_contact,
          },
        });
      ...
      <div className="grid gap-6">
        ...
        <div className="rounded-2xl border border-white/20 bg-white/70 p-4 shadow-inner">
          <div className="grid gap-3 sm:grid-cols-2">
            <Input
              value={warningInfo.customer_name}
              onChange={(e) => setWarningInfo((prev) => ({ ...prev, customer_name: e.target.value }))}
            ...
            />
            <Input
              value={warningInfo.postcode}
              onChange={(e) => setWarningInfo((prev) => ({ ...prev, postcode: e.target.value }))}
              placeholder="Postcode"
              disabled={!jobId}
            />
            <Input
              value={warningInfo.customer_contact}
              onChange={(e) => setWarningInfo((prev) => ({ ...prev, customer_contact: e.target.value }))}
          ...
          {!canContinue ? (
            <p className="mt-3 text-xs text-red-600">
              Add customer name, property address, postcode, and contact to continue.
            </p>
          ) : null}
