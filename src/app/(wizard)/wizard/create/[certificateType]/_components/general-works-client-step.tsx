...
import { assignClientToJob, createJob, saveGeneralWorksInfo } from '@/server/certificates';
...
type GeneralWorksClientStepProps = {
  clients: ClientListItem[];
  totalSteps: number;
};

export function GeneralWorksClientStep({ clients, totalSteps }: GeneralWorksClientStepProps) {
  ...
  const redirectToWizard = (jobId: string) => {
    router.push(`/wizard/create/general_works?jobId=${jobId}&clientStep=1`);
  };
  ...
  const handleSelect = (clientId: string) => {
    startTransition(async () => {
      try {
        ...
        const { jobId: createdJobId } = await createJob({ certificateType: 'general_works', clientId });
  ...
  const onSubmit = handleSubmit((values) => {
    startTransition(async () => {
      try {
        ...
        const { jobId: createdJobId } = await createJob({ certificateType: 'general_works', clientId: id });
        setJobId(createdJobId);
        setShowNewClientForm(false);
        applyClientDefaults(newClient);
        pushToast({ title: 'Client created', variant: 'success' });
  ...
  const canContinue =
    workInfo.customer_name.trim().length > 0 &&
    workInfo.property_address.trim().length > 0 &&
    workInfo.postcode.trim().length > 0 &&
    workInfo.work_date.trim().length > 0;
    ...
    startTransition(async () => {
      try {
        await saveGeneralWorksInfo({
          jobId,
          data: {
            property_address: workInfo.property_address,
            postcode: workInfo.postcode,
            work_date: workInfo.work_date,
            customer_name: workInfo.customer_name,
            engineer_name: '',
            company_name: '',
            customer_email: '',
            customer_phone: '',
        ...
        <div className="rounded-[12px] border-[0.5px] border-[var(--color-border-tertiary)] bg-[var(--color-background-primary)] p-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <Input
              ...
              onChange={(e) => setWorkInfo((prev) => ({ ...prev, customer_name: e.target.value }))}
              placeholder="Customer name"
              disabled={!jobId}
            />
            <AddressAutocompleteField
              value={workInfo.property_address}
              onValueChange={(value) => setWorkInfo((prev) => ({ ...prev, property_address: value }))}
              onAddressSelect={(address) => setWorkInfo((prev) => ({ ...prev, postcode: address.postcode || prev.postcode }))}
            ...
            />
            <Input
              value={workInfo.postcode}
              onChange={(e) => setWorkInfo((prev) => ({ ...prev, postcode: e.target.value }))}
              placeholder="Postcode"
              disabled={!jobId}
            />
            <Input
              type="date"
              value={workInfo.work_date}
              onChange={(e) => setWorkInfo((prev) => ({ ...prev, work_date: e.target.value }))}
          ...
          {!canContinue ? (
            <p className="mt-3 text-xs text-red-600">
              Add customer name, property address, postcode, and work date to continue.
            </p>
          ) : null}
