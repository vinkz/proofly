...
type CommissioningClientStepProps = {
  clients: ClientListItem[];
  totalSteps: number;
};

export function CommissioningClientStep({ clients, totalSteps }: CommissioningClientStepProps) {
  ...
  const [commissioningInfo, setCommissioningInfo] = useState({
    customer_name: '',
    property_address: '',
    postcode: '',
    customer_contact: '',
  ...
  const redirectToWizard = (jobId: string) => {
    router.push(`/wizard/create/commissioning?jobId=${jobId}&clientStep=1`);
  };
  ...
  const handleSelect = (clientId: string) => {
    startTransition(async () => {
      try {
        ...
        const { jobId: createdJobId } = await createJob({ certificateType: 'commissioning', clientId });
  ...
  const onSubmit = handleSubmit((values) => {
    startTransition(async () => {
      try {
        ...
        const { jobId: createdJobId } = await createJob({ certificateType: 'commissioning', clientId: id });
        setJobId(createdJobId);
        setShowNewClientForm(false);
        applyClientDefaults(newClient);
        pushToast({ title: 'Client created', variant: 'success' });
  ...
  const canContinue =
    commissioningInfo.customer_name.trim().length > 0 &&
    commissioningInfo.property_address.trim().length > 0 &&
    commissioningInfo.postcode.trim().length > 0;
    ...
    startTransition(async () => {
      try {
        await saveJobFields({
          jobId,
          fields: {
            customer_name: commissioningInfo.customer_name,
            property_address: commissioningInfo.property_address,
            postcode: commissioningInfo.postcode,
            customer_contact: commissioningInfo.customer_contact,
            job_address_line1: commissioningInfo.property_address,
            job_postcode: commissioningInfo.postcode,
          },
        });
        ...
        <div className="rounded-2xl border border-white/20 bg-white/70 p-4 shadow-inner">
          <div className="grid gap-3 sm:grid-cols-2">
            <Input
              ...
              onChange={(e) => setCommissioningInfo((prev) => ({ ...prev, customer_name: e.target.value }))}
            ...
            />
            <Input
              value={commissioningInfo.postcode}
              onChange={(e) => setCommissioningInfo((prev) => ({ ...prev, postcode: e.target.value }))}
              placeholder="Postcode"
              disabled={!jobId}
            />
            <Input
              value={commissioningInfo.customer_contact}
              onChange={(e) => setCommissioningInfo((prev) => ({ ...prev, customer_contact: e.target.value }))}
              placeholder="Customer contact (optional)"
              disabled={!jobId}
            />
          </div>
          {!canContinue ? (
            <p className="mt-3 text-xs text-red-600">Add customer name, property address, and postcode to continue.</p>
          ) : null}
