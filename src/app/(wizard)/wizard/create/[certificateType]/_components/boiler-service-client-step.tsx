...
import { assignClientToJob, createJob, saveBoilerServiceJobInfo } from '@/server/certificates';
...
type BoilerServiceClientStepProps = {
  clients: ClientListItem[];
  totalSteps: number;
};

export function BoilerServiceClientStep({ clients, totalSteps }: BoilerServiceClientStepProps) {
  ...
  const redirectToWizard = (jobId: string) => {
    router.push(`/wizard/create/boiler_service?jobId=${jobId}&clientStep=1`);
  };
  ...
  const handleSelect = (clientId: string) => {
    startTransition(async () => {
      try {
        ...
        const { jobId: createdJobId } = await createJob({ certificateType: 'gas_service', clientId });
  ...
  const onSubmit = handleSubmit((values) => {
    startTransition(async () => {
      try {
        ...
        const { jobId: createdJobId } = await createJob({ certificateType: 'gas_service', clientId: id });
        setJobId(createdJobId);
        setShowNewClientForm(false);
        applyClientDefaults(newClient);
        pushToast({ title: 'Client created', variant: 'success' });
  ...
  const canContinue =
    boilerInfo.customer_name.trim().length > 0 &&
    boilerInfo.property_address.trim().length > 0 &&
    boilerInfo.postcode.trim().length > 0;
      ...
      <div className="grid gap-6">
        ...
        <div className="rounded-[12px] border-[0.5px] border-[var(--color-border-tertiary)] bg-[var(--color-background-primary)] p-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <Input
              value={boilerInfo.customer_name}
              onChange={(e) => setBoilerInfo((prev) => ({ ...prev, customer_name: e.target.value }))}
              placeholder="Customer name"
              disabled={!jobId}
            />
            <AddressAutocompleteField
              value={boilerInfo.property_address}
              onValueChange={(value) => setBoilerInfo((prev) => ({ ...prev, property_address: value }))}
              onAddressSelect={(address) => setBoilerInfo((prev) => ({ ...prev, postcode: address.postcode || prev.postcode }))}
            ...
            />
            <Input
              value={boilerInfo.postcode}
              onChange={(e) => setBoilerInfo((prev) => ({ ...prev, postcode: e.target.value }))}
