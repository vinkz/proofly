...
const KNOWN_BOILER_MAKES = ['Worcester Bosch', 'Vaillant', 'Ideal', 'Baxi']
  .sort((a, b) => b.length - a.length);

const splitMakeModel = (value: string | null | undefined) => {
  const trimmed = normalizeOptionalText(value);
  if (!trimmed) return { make: null, model: null };
  const knownMake = KNOWN_BOILER_MAKES.find((make) => trimmed.toLowerCase().startsWith(make.toLowerCase()));
  if (!knownMake) return { make: trimmed, model: null };
  return {
    make: knownMake,
    model: normalizeOptionalText(trimmed.slice(knownMake.length)),
  };
};
...
async function copyBoilerServiceIdentityFromPreviousJob(params: {
  sb: Awaited<ReturnType<typeof supabaseServerServiceRole>>;
  sourceJobId: string | null | undefined;
  targetJobId: string;
  userId: string;
  ...
  const { data: fieldRows, error: fieldsErr } = await params.sb
    .from('job_fields')
  ...
  const fieldMap = (fieldRows ?? []).reduce<Record<string, string>>((acc, field) => {
    acc[String(field.field_key)] = typeof field.value === 'string' ? field.value : '';
    return acc;
  }, {});

  const { data: applianceRows, error: appliancesErr } = await params.sb
    .from('cp12_appliances')
    .select('appliance_type, location, make_model, flue_type')
    .eq('job_id', sourceJobId as JobRow['id'])
    .limit(1);
  if (appliancesErr && appliancesErr.code !== '42P01') throw new Error(appliancesErr.message);

  const appliance = applianceRows?.[0] ?? null;
  const makeModel = splitMakeModel(appliance?.make_model);
  const identityFields: Record<string, string | null> = {
    boiler_make: firstText(fieldMap.boiler_make, makeModel.make),
    boiler_model: firstText(fieldMap.boiler_model, makeModel.model),
    boiler_type: firstText(fieldMap.boiler_type, appliance?.appliance_type),
    boiler_location: firstText(fieldMap.boiler_location, fieldMap.location, appliance?.location),
    serial_number: firstText(fieldMap.serial_number),
    gas_type: firstText(fieldMap.gas_type),
    mount_type: firstText(fieldMap.mount_type),
    flue_type: firstText(fieldMap.flue_type, appliance?.flue_type),
  };

  const entries = Object.entries(identityFields)
  ...
  await persistJobFields(params.sb, params.targetJobId, entries, 'copyBoilerServiceIdentityFromPreviousJob');
}
