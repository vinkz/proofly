...
const BoilerServiceChecksSchema = z.object({
  ...
  data: z.object({
    ...
    appliance_operating_correctly: optionalText,
    appliance_conforms_standards: optionalText,
    appliance_controls_checked: optionalText,
    appliance_flueing_safe: optionalText,
    appliance_ventilation_safe: optionalText,
    emission_combustion_test: optionalText,
    burner_pressure_gas_rate_correct: optionalText,
    tightness_test_carried_out: optionalText,
    boiler_working_correctly: optionalText,
    cylinder_condition_checked: optionalText,
    programmer_controls_working: optionalText,
    co_alarm_fitted: optionalText,
    appliance_safe: optionalText,
    all_functional_parts_available: optionalText,
    warm_air_grills_working: optionalText,
    pipework_free_from_leaks: optionalText,
...
const GasWarningJobInfoSchema = z.object({
  ...
  data: z.object({
    ...
    job_address_line2: optionalLooseText,
    job_address_city: optionalLooseText,
    job_postcode: optionalLooseText,
    job_tel: optionalLooseText,
  }),
});
...
export async function saveGasWarningJobInfo(payload: z.infer<typeof GasWarningJobInfoSchema>) {
  ...
  await upsertJobAddressForJob({
    jobId,
    fields: {
      line1: data.job_address_line1 ?? data.property_address ?? undefined,
      postcode: data.job_postcode ?? data.postcode ?? undefined,
    },
    sb,
    userId: user.id,
  });
...
const Cp12ApplianceSchema = z.object({
  ...
  appliances: z
    ...
    z.object({
      ...
      flue_integrity_test: optionalText,
      flue_integrity_co2_high: optionalText,
      flue_integrity_co2_low: optionalText,
      spillage_test: optionalText,
...
export async function uploadSignature(formData: FormData) {
  ...
  const upload = await admin.storage.from('signatures').upload(path, arrayBuffer, {
    contentType: file.type || 'image/png',
    upsert: true,
  });
  if (upload.error) throw new Error(upload.error.message);
  const { data: signed, error: signedErr } = await admin.storage.from('signatures').createSignedUrl(path, 60 * 60);
  if (signedErr) throw new Error(signedErr.message);
  const url = signed?.signedUrl ?? admin.storage.from('signatures').getPublicUrl(path).data.publicUrl ?? path;
...
export async function generateGeneralWorksPdf(payload: z.infer<typeof GenerateGeneralWorksPdfSchema>) {
  ...
  const customer = resolveJobCustomer(jobContext, {
    ...
    address: typeof baseFieldMap.customer_address === 'string' ? baseFieldMap.customer_address : undefined,
    phone: typeof baseFieldMap.customer_phone === 'string' ? baseFieldMap.customer_phone : undefined,
    email: typeof baseFieldMap.customer_email === 'string' ? baseFieldMap.customer_email : undefined,
  });
  const propertyAddress = resolveJobPropertyAddress(jobContext, {
    line1: typeof baseFieldMap.property_address === 'string' ? baseFieldMap.property_address : undefined,
    line2: typeof baseFieldMap.property_address_line2 === 'string' ? baseFieldMap.property_address_line2 : undefined,
    town: typeof baseFieldMap.property_town === 'string' ? baseFieldMap.property_town : undefined,
    postcode: typeof baseFieldMap.postcode === 'string' ? baseFieldMap.postcode : undefined,
    legacy: typeof baseFieldMap.address === 'string' ? baseFieldMap.address : undefined,
  });
  const mergedFieldMap = {
    ...baseFieldMap,
    customer_name: customer.name || baseFieldMap.customer_name,
    property_address: propertyAddress.summary || baseFieldMap.property_address,
    postcode: propertyAddress.postcode || baseFieldMap.postcode,
    customer_phone: customer.phone || baseFieldMap.customer_phone,
    customer_email: customer.email || baseFieldMap.customer_email,
  };
...
export async function generateGasServicePdf(payload: z.infer<typeof GenerateGasServicePdfSchema>) {
  ...
  const publicId = buildCertificatePublicId(jobCode, 'gas_service');
  ...
  const derivedApplianceSafe = defectsFound === 'yes' ? 'NO' : defectsFound === 'no' ? 'YES' : '';
  ...
  const gasServiceFields: GasServiceFieldMap = {
    ...
    applianceOperatingCorrectly: pickText(getFieldText('appliance_operating_correctly'), getFieldText('service_visual_inspection')),
    applianceConformsStandards: getFieldText('appliance_conforms_standards'),
    applianceControlsChecked: pickText(getFieldText('appliance_controls_checked'), getFieldText('service_controls_checked')),
    operatingPressure: getFieldText('operating_pressure_mbar'),
    heatInput: getFieldText('heat_input'),
    boilerWorkingCorrectly: pickText(getFieldText('boiler_working_correctly'), getFieldText('service_visual_inspection')),
    cylinderConditionChecked: getFieldText('cylinder_condition_checked'),
    programmerControlsWorking: pickText(getFieldText('programmer_controls_working'), getFieldText('service_controls_checked')),
    coAlarmFitted: getFieldText('co_alarm_fitted'),
    applianceSafe: pickText(getFieldText('appliance_safe'), derivedApplianceSafe),
    allFunctionalPartsAvailable: getFieldText('all_functional_parts_available'),
    applianceFlueingSafe: pickText(getFieldText('appliance_flueing_safe'), getFieldText('service_flue_checked')),
