...
export async function generateReport(data: ReportJobPayload): Promise<string> {
  ...
  const completion = await openai.responses.create({
    ...
    input: [
      {
        role: 'system',
        content: `You are Proofly, an assistant who produces factual, professional compliance summaries for plumbing jobs. 
...
export async function generatePDF(payload: PdfPayload): Promise<Uint8Array> {
  ...
  drawLine('Proofly Field Report', { fontSize: 20, font: boldFont });
