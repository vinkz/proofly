...
- Complete Job Lifecycle direction adds two creation directions and one combined completion output: landlord-initiated inbound pull, engineer-initiated outbound push, and a Handover Bundle containing certificate PDFs, invoice PDF, and payment link where available.
...
- Engineer-initiated outbound push is a fourth lifecycle state rather than a separate creation route: the engineer creates or prepares the job first, then sends a tokenized link asking the landlord for missing contact, tenant, access, or preferred-date data. This should reuse `jobs.public_token` and/or `job_requests` source context instead of creating a second job.
