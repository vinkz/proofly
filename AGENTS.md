...
- Target `/jobs/[jobId]/complete` is the job-as-container checklist inserted between wizard signature completion and delivery. It reads `jobs`, `job_fields`, and final `certificates` rows to show required vs complete documents.
- Required certificates come from `jobs.cert_types`; required notices come from unsafe appliance flags in `job_fields` (`ID` or `AR`). Missing required final certificates block the Send CTA.
- Boiler service and Gas Warning Notice opened from `/jobs/[jobId]/complete` must redirect back to `/jobs/[jobId]/complete` on completion, not to dashboard or a standalone certificate page.
