...
- JobLifecycleAgent: Owns the job-as-container state model and transition rules. Target job statuses are `draft -> awaiting_landlord -> prepared -> in_progress -> issued -> delivered`; these should be additive to current job states until migrations/UI are implemented. It determines required completion rows from `jobs.cert_types`, unsafe appliance flags in `job_fields`, and final `certificates` rows, then drives the target `/jobs/[jobId]/complete` and `/jobs/[jobId]/deliver` flow without breaking the current CP12/boiler/GWN wizards.
...
- Target server actions in `src/server/jobs.ts`: `createPendingJob`, `claimPendingJob`, `createShellJob`, `sendPrefillLink`, `submitPrefillForm`, `getJobCompletionState`, `markJobInProgress`, `markJobIssued`, and `createFollowUpJob`. Add these gradually around existing `createJob`, `job_fields`, `job_requests`, and dashboard code rather than replacing current flows.
...
- Planned additive `jobs` fields for the complete lifecycle: `status`, `pending_engineer_email` or pending contact field, `claim_token`, `entry_point`, `cert_types`, `property_id`, `source_job_request_id`, and delivery timestamps such as `handover_sent_at`/`delivered_at` where useful. Add them in migrations only when the corresponding UI/server action exists.
- Planned additive `profiles` fields: `request_link_slug` for `/request/[slug]` and invoice default pricing. Prefer existing `profiles.standard_rates` for CP12/boiler/combo rates; use a single `default_rate` only as a simple fallback if needed.
...
- Step 2 onboarding collects profession (dropdown + manual fallback), company name, engineer name, engineer ID card number, and Gas Safe registration; these are persisted to `profiles` for certificate defaults. Preserve Gas Safe ID validation direction: business registration is 6 digits and individual engineer ID card number is 7 digits.
...
- `/dashboard` must surface pending `job_requests` above the calendar as the request inbox. Renewal requests from public job links, standalone landlord requests, and personal-link requests use the same cards and scheduling path.
- `/dashboard` should also expose an awaiting-landlord outbox for engineer-created shell jobs where a pre-fill link has been sent. These cards show who the link was sent to, when, current response state, copy/resend actions, and the `Fill in myself` escape hatch.
