...
- ProfileAgent: Maintains `profiles` table fields (`trade_types`, `certifications`, `onboarding_complete`, personal basics, company details, Gas Safe IDs, request link slug, invoice defaults) via `src/server/profile.ts`, enforces guards in `RequireAuth` (redirects incomplete users to `/onboarding?step=N` based on missing profile fields, while allowing `/onboarding` and `/settings`). Profile setup must preserve Gas Safe ID rules: 6-digit business registration number and 7-digit individual ID card number where validation is enforced. Future request-link work should generate a permanent `request_link_slug` per engineer; invoice pricing currently uses `profiles.standard_rates`, with a simple `default_rate` only as a possible compatibility/fallback field.
- JobRequestAgent: Owns the bidirectional request/link lifecycle for landlord-submitted work requests and engineer-created jobs awaiting landlord data. It validates landlord request/pre-fill forms, classifies property/history-aware requests as `request_type = 'new_job'` or `request_type = 'renewal'`, creates/updates `job_requests` rows, triggers engineer/landlord notifications, and keeps request status moving through `pending -> scheduled -> completed -> dismissed`. It must support the three entry points: public landlord request with engineer lookup (`/request`, currently compatible with `/request-job`), engineer-created shell job awaiting landlord pre-fill (`/prefill/[jobId]?token=...` target), and engineer personal request link (`/request/[slug]` target). If the landlord names an unregistered engineer, store the pending work on `job_requests` with `claim_token`/`claim_token_expires_at` as the canonical signup link rather than creating an ownerless job or relying on email matching; the engineer may register with a different email. Public claim/pre-fill routes must validate tokens server-side through route handlers/server actions and must not add broad public RLS reads on `jobs`. Do not build a public browseable engineer directory.
...
- DashboardAgent: Owns the operational dashboard inbox/outbox model. Inbound landlord demand appears in the request inbox above scheduled work. Engineer-sent pre-fill links and jobs in `awaiting_landlord` appear in an outbox/awaiting-response surface with status, pre-fill URL, expiry, and a `Fill in myself` escape hatch. Inbox/outbox visibility is milestone-critical and should take priority over calendar polish because the new request flow is not useful if engineers cannot see inbound work or jobs waiting on landlord data. The first milestone is not complete until an engineer can see a landlord-submitted request, schedule/open the prepared job, and see any jobs still waiting for landlord pre-fill from the same dashboard. New engineers should be nudged to send a job/pre-fill link to a landlord when they have no prepared work yet, but this must be framed as an optional activation shortcut, not required onboarding.
...
- External integrations: Supabase (auth/storage), PDF generation via `pdf-lib`, address lookup via Ideal Postcodes, transactional email via Resend, and error monitoring via Sentry; isolate integration code under server/API routes or framework instrumentation files.
...
- Entry Point 2, engineer-created job: engineer starts from `/jobs/new` exactly as today. They can fill Step 1 themselves and move the job to `prepared`, or create a shell job in `awaiting_landlord` and send a scoped pre-fill link. This preserves real-world work received by phone, WhatsApp, email, repeat clients, urgent visits, and site conversations. The job request system must never replace or block manual creation.
- Entry Point 3, engineer personal request link: each engineer should eventually have a permanent `/request/[request_link_slug]` URL generated from their profile. They can share it from settings, WhatsApp, business cards, or Google Business profile. Landlords using this link skip engineer lookup because the slug already scopes the request to the engineer.
...
- Engineer-created `awaiting_landlord` jobs need an escape hatch. Dashboard cards must expose `Fill in myself`, which clears the awaiting state back to `draft`/manual prep without losing any already-collected data.
...
- Target `/request` handles landlord-initiated requests where the landlord enters the engineer email/phone/Gas Safe reference they already have. It may create an assigned request/job, or an unclaimed pending job with `claim_token` if the engineer is not registered.
- Target `/request/[slug]` is the engineer personal request link. It skips engineer lookup and writes the request/job against the engineer resolved from `profiles.request_link_slug`.
...
- Target `/jobs/[jobId]/complete` is the job-as-container checklist inserted between wizard signature completion and delivery. It is an authenticated app route, not a landlord/public route. Build this after the signup/profile, lifecycle migration, Resend/request routes, and dashboard inbox/outbox foundations are stable because it needs `jobs.status`, `jobs.cert_types`, final `certificates` rows, and a dashboard CTA path into issued/prepared jobs.
- `/jobs/[jobId]/complete` should be implemented additively. Existing CP12/boiler/GWN PDF generation must keep working while final wizard redirects are migrated one by one. Legacy jobs with missing `cert_types` should infer required rows from `jobs.job_type` and existing certificate rows where possible rather than crashing or hiding the job.
- Core server action: `getJobCompletionState(jobId)` in `src/server/jobs.ts`. It must validate authenticated ownership, read `jobs`, `job_fields`, and `certificates`, then return checklist rows split into `required`, `completed`, and `ready` states. Do not put PDF rendering in this action; it should only inspect existing job/certificate state and produce UI state.
...
- Boiler service and Gas Warning Notice opened from `/jobs/[jobId]/complete` must redirect back to `/jobs/[jobId]/complete` on completion, not to dashboard or a standalone certificate page. Thread `jobId` through the wizard as a route/search param and revalidate the completion route after storing the final certificate row.
...
- Current CP12 prompt/deep-link behaviour may remain until the completion screen exists, but new work should move toward the checklist model rather than adding more one-off post-certificate prompts.
...
- CP12 currently includes a CTA on Step 3 when "Warning notice issued" is YES that deep-links to Gas Warning Notice using the same jobId. Target direction is to retire this as the primary path once `/jobs/[jobId]/complete` exists; unsafe appliance flags should instead make Gas Warning Notice a required completion checklist item.
...
- `/dashboard` must surface pending `job_requests` above the calendar as the request inbox. Renewal requests from public job links, standalone landlord requests, and personal-link requests use the same cards and scheduling path. This inbox is the primary handoff from landlord demand to engineer work; it must not be hidden behind calendar tabs or completed-job history.
- `/dashboard` should also expose an awaiting-landlord outbox for engineer-created shell jobs where a pre-fill link has been sent. These cards show who the link was sent to, when, current response state, copy/resend actions when implemented, pre-fill URL/expiry, and the `Fill in myself` escape hatch. If the pre-fill link is expired or missing, the card should still show the manual escape hatch instead of dead-ending.
...
- Selected-day job rows should show job type labels, client/title, address, scheduled time, status badge, and the correct action (`Prepare`, `Start`, `Continue`, `Complete`, `Deliver`, `Open`, or `Open PDF`). CP12 jobs with incomplete Step 1 prep should link to the prepare-only path.
...
- Upcoming calendar jobs compute a prep state from saved Step 1 fields. Unprepared CP12 jobs show `Prepare`; `awaiting_landlord` jobs show `Awaiting landlord` plus `Fill in myself`; `prepared` jobs show `Start` and call `markJobInProgress`; `in_progress` jobs show `Continue`; `issued` jobs show `Deliver` once the completion checklist is resolved.
