...
- DashboardAgent: Owns the operational dashboard request handoff. The top card remains the general welcome/action hero with `New Job` and `Create invoice`; the next section is `Landlord Requests`, showing only the latest pending request plus a badge count and a `/requests` link for the full list. If there are no requests, show the engineer’s scoped request link with copy/open actions so they can send it to a landlord; landlords using this link must not need to enter engineer details. Dashboard rows now treat `issued` as a review/send state, showing `Review & send` and routing to `/jobs/[jobId]/complete`; legacy `completed` jobs still open the PDF view. Request visibility is milestone-critical because the new request flow is not useful if engineers cannot see inbound work from the dashboard.
...
- Entry Point 3, engineer personal request link: each engineer should have a permanent `/request/[request_link_slug]` URL generated from their profile on demand. They can copy it from the dashboard empty request state, send it from `/jobs/new`, or share it through WhatsApp, business cards, or Google Business profile. Landlords using this link skip engineer lookup because the slug already scopes the request to the engineer.
...
- Target `/request/[slug]` is the engineer personal request link. It skips engineer lookup and writes the request/job against the engineer resolved from `profiles.request_link_slug`; this is the preferred link sent from `/jobs/new` and shown in the dashboard empty request state.
