...
- RLS hardening for `public.job_requests` is captured in `supabase/migrations/20260513130000_enable_job_requests_rls.sql`: enable RLS, add a read-only authenticated engineer owner/matching-email policy, and keep public request creation/claim/schedule/dismiss/pre-fill flows behind server actions with service-role writes and explicit validation. Do not add broad anon read/write policies to make public request forms work.
